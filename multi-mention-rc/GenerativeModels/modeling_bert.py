...
class BertForQuestionAnswering(nn.Module):
    ...
    def forward(self, batch, global_step=-1):
        ...
        if len(batch) == 7:
            ...
            span_mask = answer_mask * (switch == 0).type(torch.FloatTensor).to(device)
            ...
            if self.loss_type=='first-only':
                ...
                if numpy.random.random()<min(global_step/self.tau, 0.8): # TODO
    ...
    def _take_min(self, loss_tensor):
        ...
        return torch.sum(torch.min(
            loss_tensor + 2*torch.max(loss_tensor)*(loss_tensor==0).type(torch.FloatTensor).to(device), 1)[0])
