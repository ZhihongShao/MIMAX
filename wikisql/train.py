...
def train(args, library, dataset, prior_net, posterior_net, logger):
    ...
    for epoch in train_iterator:
        ...
        while True:
            try:
                ...
                try:
                    dataset.prepare_next_shard(
                        posterior_net,
                        prior_net,
                        dynamic_preparation=False if args.loss_type == 'hard-em-thres' else True,
                        for_posterior=True,
            ...
            for model_type, model in zip(['posterior'], [posterior_net]):
                if args.loss_type == 'hard-em-thres':
            ...
            if args.save_steps > 0 and global_step % args.save_steps == 0:
                ...
                if (
                    ...
                    eval_dataset = NeRdDataset.NeRdDataset(args.predict_dev_file, args.predict_dev_table_file, args.predict_dev_db_file, library, NeRdDataset.NeRdDataset.EVAL_MODE, args.local_rank)
                    ...
                    eval_results, eval_programs, eval_guid2programs = predict.predict(args, eval_dataset, posterior_net_cpy, logger, None)
                ...
                for optimizer, scheduler, path in zip([(None if args.plm_adaptive_lr else posterior_optimizer)], [(None if args.plm_adaptive_lr else posterior_scheduler)], [posterior_path]):
