...
# Load data -----------------------------------------------------------------------------------------------
def load_wikisql(args, path_wikisql, version, toy_model, toy_size, bert=False, no_w2i=False, no_hs_tok=False, aug=False):
    # Get data
    train_data, train_table = load_wikisql_data(path_wikisql, version,
                                                mode='train' if args.test is None else args.test,
                                                toy_model=toy_model, toy_size=toy_size, no_hs_tok=no_hs_tok, aug=aug)
    dev_data, dev_table = load_wikisql_data(path_wikisql, None,
                                            mode='dev' if args.test is None else args.test,
                                            toy_model=toy_model, toy_size=toy_size, no_hs_tok=no_hs_tok)
...
def load_wikisql_data(path_wikisql, version, mode='train', toy_model=False, toy_size=10, no_hs_tok=False, aug=False):
    """ Load training sets
    """
    if aug:
        mode = "aug.{mode}"
    ...
    path_sql = os.path.join(path_wikisql, mode+'_tok{}.jsonl'.format('' if version==None else '_'+version))
    if no_hs_tok:
        path_table = os.path.join("/home/sewon/data/wikisql", mode + '.tables.jsonl')
    else:
        path_table = os.path.join("/home/sewon/data/wikisql", mode+'.tables.jsonl')
...
def get_fields_1(t1, tables, train=False, no_hs_t=False, no_sql_t=False):

    nlu1 = t1['question']
    nlu_t1 = t1['question_tok']
    tid1 = t1['table_id']
    sql_i1 = t1['sql'] #[t1['sql'][0] for _ in range(30)]
    sql_q1 = t1['sql'] # [t1['query'][0] for _ in range(30)]
    wvi_corenlp = t1['wvi_corenlp'] #[t1['wvi_corenlp'][0] for _ in range(30)]

    if type(sql_i1) == type(sql_q1) == dict:
        sql_i1 = [sql_i1]
        sql_q1 = [sql_q1]
        wvi_corenlp = [wvi_corenlp]

    for (sql, wvi) in zip(sql_i1, wvi_corenlp):
        if wvi is not None and len(sql['conds']) != len(wvi):
    ...
    return nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1, wvi_corenlp

def get_fields(t1s, tables, train=False, no_hs_t=False, no_sql_t=False):

    tot_N = 0
    nlu, nlu_t, tid, sql_i, sql_q, sql_t, tb, hs_t, hs, wvi = [], [], [], [], [], [], [], [], [], []
    for t1 in t1s:
        if no_hs_t:
            nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1, wvi1 = get_fields_1( \
                                        t1, tables, train, no_hs_t, no_sql_t)
        else:
            nlu1, nlu_t1, tid1, sql_i1, sql_q1, sql_t1, tb1, hs_t1, hs1, wvi1 = get_fields_1( \
                                        t1, tables, train, no_hs_t, no_sql_t)
        tot_N += len(sql_i1)
    ...
    # if train:
    #     n_each = 50
    #     while sum([len(sql_i1) for sql_i1 in sql_i]) > 300:
    #         for idx in range(len(sql_i)):
    #             if len(sql_i[idx])<=n_each: continue
    #             indices = np.random.permutation(range(len(sql_i[idx])))[:n_each]
    #             sql_i[idx] = [sql_i[idx][i] for i in indices]
    #             sql_q[idx] = [sql_q[idx][i] for i in indices]
    #             wvi[idx] = [wvi[idx][i] for i in indices]
    #         n_each = n_each - 10

    #     return nlu, nlu_t, sql_i, sql_q, sql_t, tb, hs_t, hs, wvi
...
def encode(lstm, wemb_l, l, return_hidden=False, hc0=None, last_only=False):
    ...
    try:
        packed_wemb_l = nn.utils.rnn.pack_padded_sequence(wemb_l[perm_idx, :, :].contiguous(),
                                                      l[perm_idx],
                                                      batch_first=True)
    except Exception:
        print ("Error while going through lstm")
        from IPython import  embed;  embed()
        exit()
    torch.cuda.synchronize()
    # Time to encode
    ...
    wenc = wenc[perm_idx_inv].contiguous()
    ...
    if return_hidden:
        ...
        hout = hout[:, perm_idx_inv].to(device).contiguous()
        cout = cout[:, perm_idx_inv].to(device).contiguous()  # Is this correct operation?
...
def get_g(sql_i):
    ...
    for b, psql_i1 in enumerate(sql_i):
        g_sc.append( [i["sel"] for i in psql_i1] )
        g_sa.append( [i["agg"] for i in psql_i1] )
...
def get_g_wvi_corenlp(t):
    g_wvi_corenlp = []
    for t1 in t:
        wvi_corenlp = t1['wvi_corenlp']
        if type(t1['sql']) == dict:
            wvi_corenlp = [wvi_corenlp] # FIXME
        g_wvi_corenlp.append( wvi_corenlp )
        ...
        def _get(nlu_t1):
            ...
            for (i, token) in enumerate(nlu_t1):
                ...
                for sub_token in sub_tokens:
                    tt_to_t_idx1.append(i)
                    nlu_tt1.append(sub_token)  # all_doc_tokens are further tokenized using WordPiece tokenizer
            return nlu_tt1, tt_to_t_idx1, t_to_tt_idx1

        hds1 = hds[b]
        nlu_tt1, tt_to_t_idx1, t_to_tt_idx1 = _get(nlu_t1)


        # [CLS] nlu [SEP] col1 [SEP] col2 [SEP] ...col-n [SEP]
        # 2. Generate BERT inputs & indices.
        query_length = int(0.9*max_seq_length)
        query_turn = False
        while True:
            ...
            if len(tokens1)<=max_seq_length:
                break
            if query_turn:
                nlu_t1 = nlu_t1[:query_length]
                query_length = int(0.9*query_length)
                query_turn = False
                nlu_tt1, tt_to_t_idx1, t_to_tt_idx1 = _get(nlu_t1)
            else:
                if len(hds1)>1: hds1 = hds1[:-2]
                query_turn = True
...
def get_inputs(tokenizer, nlu_t, hds, max_seq_length):
    """
    Here, input is toknized further by WordPiece (WP) tokenizer and fed into BERT.
    ...
    # Convert to tensor
    all_input_ids = pad_sequence([torch.tensor(line, dtype=torch.long) for line in input_ids], batch_first=True, padding_value=0).to(device)
    all_input_mask = pad_sequence([torch.tensor(line, dtype=torch.long) for line in input_mask], batch_first=True, padding_value=0).to(device)
    all_segment_ids = pad_sequence([torch.tensor(line, dtype=torch.long) for line in segment_ids], batch_first=True, padding_value=0).to(device)
    # all_input_ids = torch.tensor(input_ids, dtype=torch.long).to(device)
    # all_input_mask = torch.tensor(input_mask, dtype=torch.long).to(device)
    # all_segment_ids = torch.tensor(segment_ids, dtype=torch.long).to(device)
    ...
    return all_input_ids, all_input_mask, all_segment_ids, tokens, i_nlu, i_hds, \
           l_n, l_hpu, l_hs, \
           nlu_tt, t_to_tt_idx, tt_to_t_idx
...
def pred_wvi_se(wn, s_wv):
    ...
    mask = torch.zeros_like(s_wv_st, device=s_wv_st.device, dtype=torch.float32)
    mask = mask.scatter_(2, pr_wvi_st_idx.unsqueeze(2), 1.0)
    mask = torch.cumsum(mask, 2)
    pr_wvi_ed_idx = (s_wv_ed - (1 - mask) * 1e11).argmax(dim=2)
...
def convert_pr_wvi_to_string(pr_wvi, nlu_t, nlu_wp_t, wp_to_wh_index, nlu, return_wvi_tok=False):
    """
    ...
    for b, pr_wvi1 in enumerate(pr_wvi):
        ...
        for i_wn, pr_wvi11 in enumerate(pr_wvi1):
            ...
            pr_wvi_tok1.append((st_wh_idx, ed_wh_idx))
        ...
        pr_wvi_tok.append(pr_wvi_tok1)

    if return_wvi_tok:
        return pr_wv_str, pr_wv_str_wp, pr_wvi_tok
    else:
...
def get_where_value_indices_in_nlu_tokens(where_values, nlu_tokens):
    def get_end(tokens, target):
        for j in range(1, len(tokens) + 2):
            if j == len(tokens) + 1 or not target.replace(" ", '').startswith("".join(tokens[:j])):
                break
        if target.replace(" ", '') == "".join(tokens[:j - 1]):
            return j - 1
        else:
            return None
    def match(tokens, target):
        if isinstance(target, str):
            for i in range(len(tokens)):
                if target.startswith(tokens[i]):
                    j = get_end(tokens[i:], target)
                    if j is not None:
                        return (i, i + j - 1)
        if isinstance(target, str):
        ...
        try:
            ...
            for i in range(len(tokens)):
                try:
                    token = float(tokens[i].replace(",", ''))
                    if np.isclose(target, token):
                        return (i, i)
                except:
                    pass
        except:
        ...
        return None
    g_wvi1_corenlp = []
    nlu_tokens = [tok.lower() for tok in nlu_tokens]
    for value in where_values:
        if isinstance(value, str):
            value = value.lower()
        inv = match(nlu_tokens, value)
        if inv is None:
            return None
        else:
            g_wvi1_corenlp.append(inv)
...
def save_for_evaluation(path_save, results, dset_name, ):
    path_save_file = os.path.join(path_save, 'results_{}.jsonl'.format(dset_name))
...
def save_for_evaluation_aux(path_save, results, dset_name, ):
    path_save_file = os.path.join(path_save, 'results_aux_{dset_name}.jsonl')
    with open(path_save_file, 'w', encoding='utf-8') as f:
        for i, r1 in enumerate(results):
            json_str = json.dumps(r1, ensure_ascii=False, default=json_default_type_checker)
            json_str += '\n'
...
def generate_sql_q1(sql_i1, tb1):
    ...
    sql_query_part1 = 'SELECT {select_agg}({select_header}) '


    where_num = len(sql_i1['conds'])
    if where_num == 0:
        sql_query_part2 = 'FROM {select_table}'
        # sql_plus_query_part2 = 'FROM {select_table}'

    else:
        sql_query_part2 = 'FROM {select_table} WHERE'
        # sql_plus_query_part2 = 'FROM {select_table_refined} WHERE'
        # ----------------------------------------------------------------------------------------------------------
        for i in range(where_num):
            ...
            sql_query_part2 += " {where_header} {where_op} {where_str}"
