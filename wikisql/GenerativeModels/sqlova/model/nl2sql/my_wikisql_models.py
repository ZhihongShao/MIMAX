...
from GenerativeModels.sqlova.utils.utils import topk_multi_dim
from GenerativeModels.sqlova.utils.utils_wikisql import *
...
class SCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_sc=False):
        ...
        att_h = torch.bmm(wenc_hs, self.W_att(wenc_n).transpose(1, 2)).contiguous()
        ...
        c_n = torch.mul(p_n.unsqueeze(3), wenc_n.unsqueeze(1)).contiguous().sum(dim=2)
...
class SAP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, pr_sc, show_p_sa=False):
        ...
        try:
            assert all([sc<hs for sc, hs in zip(pr_sc, l_hs)])
        except Exception:
            print ("line 414 in model")
        ...
        att = torch.bmm(self.W_att(wenc_n), wenc_hs_ob.unsqueeze(2)).squeeze(2).contiguous()
        mask = torch.zeros(att.size(), dtype=torch.uint8)
        #   Penalty on blank parts
        for b, l_n1 in enumerate(l_n):
            if l_n1 < mL_n:
                assert 0 <= b < att.size(0)
                assert 0 <= l_n1 < att.size(1)
                mask[b, l_n1:] = 1
        ...
        #       -> [bS, mL_n, 100] -> [bS, 100]
        try:
            c_n = torch.mul(wenc_n, p.unsqueeze(2).expand_as(wenc_n).contiguous()).sum(dim=1)
        except Exception:
            print ("Model line 447")
            from IPython import embed; embed()
            exit()
...
class WNP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wn=False):
        ...
        c_hs = torch.mul(wenc_hs, p_h.unsqueeze(2)).contiguous().sum(1)
        ...
        c_n = torch.mul(wenc_n, p_n.unsqueeze(2).expand_as(wenc_n)).contiguous().sum(dim=1)
...
class WCP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, show_p_wc, penalty=True):
        ...
        att = torch.bmm(wenc_hs, self.W_att(wenc_n).transpose(1, 2)).contiguous()
        ...
        c_n = torch.mul(wenc_n, p).contiguous().sum(2)  # -> [b, hs, dim], c_n for each header.
...
class WOP(nn.Module):
    ...
    def forward(self, wemb_n, l_n, wemb_hpu, l_hpu, l_hs, wn, wc, wenc_n=None, show_p_wo=False):
        ...
        #  --> [B, 4, mL_n, dim]
        #  --> [B, 4, dim]
        c_n = torch.mul(wenc_n.unsqueeze(1), p.unsqueeze(3)).contiguous().sum(dim=2)
...
def Loss_sw_se(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
    ...
    :return:
    """
    loss = Loss_sc(s_sc, g_sc, reduction)
    loss += Loss_sa(s_sa, g_sa, reduction)
    loss += Loss_wn(s_wn, g_wn, reduction)
    loss += Loss_wc(s_wc, g_wc, reduction)
    loss += Loss_wo(s_wo, g_wn, g_wo, reduction)
    loss += Loss_wv_se(s_wv, g_wn, g_wvi, reduction)
    return loss

def Loss_sc(s_sc, g_sc, reduction):
    loss = F.cross_entropy(s_sc, torch.tensor(g_sc).to(device), reduction=reduction)
    return loss


def Loss_sa(s_sa, g_sa, reduction):
    loss = F.cross_entropy(s_sa, torch.tensor(g_sa).to(device), reduction=reduction)
    return loss

def Loss_wn(s_wn, g_wn, reduction):
    loss = F.cross_entropy(s_wn, torch.tensor(g_wn).to(device), reduction=reduction)
    return loss

def Loss_wc(s_wc, g_wc, reduction):
    ...
    # Construct prob.
    p = F.sigmoid(s_wc)
    loss = F.binary_cross_entropy(p, im, reduction=reduction)
    if reduction=='none':
        return torch.mean(loss, dim=1)
    return loss


def Loss_wo(s_wo, g_wn, g_wo, reduction):

    # Construct index matrix
    loss = []
    for b, g_wn1 in enumerate(g_wn):
        if g_wn1 == 0:
            loss.append(torch.zeros([1]).cuda())
            continue
        g_wo1 = g_wo[b]
        s_wo1 = s_wo[b]
        l = F.cross_entropy(s_wo1[:g_wn1], torch.tensor(g_wo1).to(device))
        loss.append(l.unsqueeze(dim=0))
    if reduction=='none':
        return torch.cat(loss, dim=0)
    return sum([torch.sum(l) for l in loss])

def Loss_wv_se(s_wv, g_wn, g_wvi, reduction):
    """
    s_wv:   [bS, 4, mL, 2], 4 stands for maximum # of condition,
                            2 tands for start & end logits.
    ...
    for b, g_wvi1 in enumerate(g_wvi):
        ...
        l = F.cross_entropy(s_wv[b,:g_wn1,:,0], g_st1)
        # print("st_login: ", s_wv[b,:g_wn1,:,0], g_st1, loss)
        # loss from the end position
        l += F.cross_entropy(s_wv[b,:g_wn1,:,1], g_ed1)
