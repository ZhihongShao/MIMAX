...
def Loss_sw_se(s_sc, s_sa, s_wn, s_wc, s_wo, s_wv, g_sc, g_sa, g_wn, g_wc, g_wo, g_wvi,
    ...
    :return:
    """
    loss1 = Loss_sc(s_sc, g_sc, reduction)
    loss2 = Loss_sa(s_sa, g_sa, reduction)
    loss3 = Loss_wn(s_wn, g_wn, reduction)
    loss4 = Loss_wc(s_wc, g_wc, reduction)
    loss5 = Loss_wo(s_wo, g_wn, g_wo, reduction)
    loss6 = Loss_wv_se(s_wv, g_wn, g_wvi, reduction)
    '''
    loss11 = Loss_sc(s_sc, g_sc, 'sum')
    loss21 = Loss_sa(s_sa, g_sa, 'sum')
    loss31 = Loss_wn(s_wn, g_wn, 'sum')
    loss41 = Loss_wc(s_wc, g_wc, 'sum')
    loss51 = Loss_wo(s_wo, g_wn, g_wo, 'sum')
    loss61 = Loss_wv_se(s_wv, g_wn, g_wvi, 'sum')
    for (l, l1) in zip([loss1, loss2, loss3, loss4, loss5, loss6], [loss11, loss21, loss31, loss41, loss51, loss61]):
        print (torch.sum(l), l1)
    from IPython import embed; embed()
    quit()'''
...
def Loss_wo(s_wo, g_wn, g_wo, reduction):
    ...
    for b, g_wn1 in enumerate(g_wn):
        ...
        g_wo1 = g_wo[b]
        s_wo1 = s_wo[b]
        loss.append(F.cross_entropy(s_wo1[:g_wn1], torch.tensor(g_wo1).to(device)).unsqueeze(dim=0))
    if reduction=='none':
