...
class Library:
    def __init__(self, max_num_args, max_program_depth, prior_textual_tokens_dir, post_textual_tokens_dir):
        if 'bart' in prior_textual_tokens_dir.lower():
            self.prior_tokenizer = BartTokenizer.from_pretrained(prior_textual_tokens_dir)
        else:
            raise NotImplementedError()
        if 'bert' in post_textual_tokens_dir.lower() or 'electra' in post_textual_tokens_dir.lower():
            self.post_tokenizer = BertTokenizer.from_pretrained(post_textual_tokens_dir)
        else:
            raise NotImplementedError()
        print("Finish loading vocabulary...", flush=True)
    ...
    def get_cls_token_id(self, is_prior=False):
        ...
        if is_prior:
            return self.prior_tokenizer.cls_token_id
        else:
            return self.post_tokenizer.cls_token_id

    def get_sep_token_id(self, is_prior=False):
        assert self._is_finalized
        if is_prior:
            return self.prior_tokenizer.sep_token_id
        else:
            return self.post_tokenizer.sep_token_id

    def get_bos_token_id(self, is_prior=False):
        assert self._is_finalized
        if is_prior:
            return self.prior_tokenizer.bos_token_id
        else:
            return self.post_tokenizer.bos_token_id
    ...
    def get_pad_token_id(self, is_prior=False):
        ...
        if is_prior:
            ...
            return self.post_tokenizer.pad_token_id

    def get_eof_token_id(self, is_prior=False):
        assert self._is_finalized
        if is_prior:
            return self.prior_tokenizer.eof_token_id
        else:
            return self.post_tokenizer.eof_token_id

    def get_eos_token_id(self, is_prior=False):
        assert self._is_finalized
        if is_prior:
            return self.prior_tokenizer.eos_token_id
        else:
            return self.post_tokenizer.eos_token_id
    ...
    def get_num_tokens(self, is_prior):
        assert self._is_finalized
        if is_prior or self.get_textual_op_lst(is_prior=is_prior):
            return len(self.prior_tokenizer if is_prior else self.post_tokenizer)
        else:
    ...
    def get_reserved_token_id_offset(self, is_prior=False):
        assert self._is_finalized
        if is_prior:
            return self.prior_tokenizer.vocab_size + len(self.additional_tokens)
        else:
            return self.post_tokenizer.vocab_size + len(self.additional_tokens)
    ...
    def is_index(self, token_id):
        assert self._is_finalized
        return token_id >= self.get_num_tokens(is_prior=True)
    ...
    def finalize(self, additional_tokens):
        ...
        for tokenizer in [self.prior_tokenizer, self.post_tokenizer]:
    ...
    def convert_tokens_to_ids(self, tokens, add_eos_token_id=False, is_prior=False):
        ...
        for token in tokens:
            ...
            if res is not None:
                token_ids.append(int(res.groups()[0]) + self.get_num_tokens(is_prior=True))
            else:
    ...
    def tokenize(self, text, is_prior=False):
        return self.prior_tokenizer.tokenize(text) if is_prior else self.post_tokenizer.tokenize(text)
    ...
    def encode(self, text, add_bos_token_id=False, add_eos_token_id=False, is_prior=False):
        ...
        for token in text.split():
            ...
            if res is not None:
                ...
                token_ids.extend(tokenizer.convert_tokens_to_ids(tokenizer.tokenize(token), is_prior=is_prior))
        if add_bos_token_id:
    ...
    def decode(self, token_ids, remove_special_tokens=True, is_prior=False):
        ...
        for token_id in token_ids:
            ...
            if self.is_index(token_id):
                tokens.append('<index:{}>'.format(token_id - self.get_num_tokens(is_prior=True)))
            else:
